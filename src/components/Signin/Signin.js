...
class Signin extends Component {
  constructor(props) {
    ...
    this.showWait = false;
    if (document.cookie.indexOf("firstCookie") == -1) {
      document.cookie = "firstCookie=1";
    } else if (
      document.cookie.indexOf("firstCookie") != -1 &&
      document.cookie.indexOf("secondCookie") == -1
    ) {
    ...
    this.firstTime = (
      <h7>
        ...
        <br />
        This website is hosted on a free server. So the first
        login might take a while to load as it wakes the server.
        <br />
        Thanks for waiting!
      </h7>
    );
    this.returningUser = <h7>Register or login to access.</h7>;
  ...
  componentDidMount() {
    console.log("here");
    fetch("https://upper-inukshuk-26953.herokuapp.com/", {
      method: "post",
    })
      .then((response) => response.text())
      .then((resData) => {
        ...
        <div class="sidenav">
          ...
          >
            ...
            <div class="title-padding">
              ...
              {this.showWait ? this.firstTime : this.returningUser}
            </div>
          </div>
        </div>
